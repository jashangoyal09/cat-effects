...
//  }
      ...
      case GET -> Root / "movies" :? DirectorQueryParamMatcher(director) +& YearQueryParamMatcher(maybeYear) =>
        ...
        maybeYear match {
          case Some(validatedYear) =>
            validatedYear
            val moviesByDirectorAndYear = moviesByDirector.filter(_.year == validatedYear.getValue)
  ...
  val directorDetailsDB: mutable.Map[Director, DirectorDetails] =
    mutable.Map(Director("Zack", "Snyder") -> DirectorDetails("Zack", "Synder", "superhero"))
